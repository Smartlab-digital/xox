(() => {
  ...
  window.XOXAPI = {
    ...
    verifyEmail: async token => { const result = await request('verify-email', {method: 'POST', data: {token}}); if (result.user) user = result.user; return result; },
    completeRegistration: async password => { const result = await request('complete-registration', {method: 'POST', data: {password}}); user = result.user; return result; },
